...
def hefty_hogs(player_score, opponent_score):
    ...
    while(opponent_score != 0):
        ...
        func = digit_fn(last_digit) 
        num = func(player_score)
        player_score = num
...
def play(strategy0, strategy1, score0=0, score1=0, dice=six_sided,
    ...
    goal:       The game ends and0
   someone wins when this score is reached.
    ...
    while(score0 < goal and score1 < goal):
        if(who == 0):
            x0 = strategy0(score0,score1) 
            score0 = score0 + take_turn(x0,score0,score1,dice)
            score0 = score0 + hog_pile(score0,score1)


        else:
            x1 = strategy1(score1,score0)
            score1 = score1 + take_turn(x1,score1,score0,dice)
...
def announce_lead_changes(score0, score1, last_leader=None):
    ...
    if(score0>score1):
        ...
        message = "Player 0 takes the lead by " + str(difference)
    else:
        difference = score1 - score0
        message = "Player 1 takes the lead by " + str(difference)
    ...
    if(score0 == score1):
        ...
        if(score0 > score1):
            ...
            return 1, message
    if(score0 > score1 and last_leader != 1):
        return 0,None
    elif(score0 > score1 and last_leader == 1):
        return 0, message
    elif(score1 > score0 and last_leader != 0):
        return 1,None
    else:
        return 1, message
...
def max_scoring_num_rolls(dice=six_sided, total_samples=1000):
    ...
    dummy = 0
    while(counter<end):
        dummy = make_averaged(roll_dice)(counter,dice)
        if(dummy>make_averaged(roll_dice)(max,dice)):
           max = counter
        counter += 1
    return max
    # END PROBLEM 9
...
def hefty_hogs_strategy(score, opponent_score, threshold=8, num_rolls=6):
    ...
    """
    # BEGIN PROBLEM 10
    dummy = hefty_hogs(score, opponent_score)
    if (dummy>=threshold):
        return 0
    else:
        return num_rolls
...
def hog_pile_strategy(score, opponent_score, threshold=8, num_rolls=6):
    ...
    """
    # BEGIN PROBLEM 11
    hefty_dummy = hefty_hogs(score,opponent_score)
    if(hefty_dummy>=threshold or hog_pile(score+hefty_dummy,opponent_score)):
        return 0
    else:
        return num_rolls
